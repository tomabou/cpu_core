...
9/15
cuはうまく動かない
screenは動いて、テキストの送信に成功した
バイナリを送信できるのか確認する
catで表示してコマンドラインで流し込めば良いということがわかった

done
fiboをコンパイルしてそのままnibu上で実行出来た
...
pseudoを追加
divをどうするか
マンデルブロを一気に計算して出力すると、256*256=60kでメモリが足りない
一ビットづつ計算して順次出力する形式でないとまずい
FIFOも256しかないのでバッファオーバーフローを起こす

done
load immをluiに対応するように修正した
psuedo decodeを追加した

todo
メモリユニットが任意時間停止できるようにする
  * DRAMコントローラーの追加
  * sb,sh命令の実行
  * uartのバッファオーバーフロー対処
後ろ２つはソフトウェア側で対処可能ではあるuartのバッファオーバーフローのほうが簡単か？
divはとりあえず実装しないで -mno-divをつける -> done
mulの実装 -> asmを書いていないので書く
4オペランド命令の実装
マンデルブロの実装
la以外に　load store global命令を実装する -> done

mulを書いてみたらクロックギリギリになった
メモリから読んだものをそのままforwardするのはやめた方が良いかも
メモリのhazardは1クロック伸ばす
mulhsuのはipがないので、mulを計算した後に引き算をしなくてはならない
これは遅いかも


9/17
負の数が出てきたときの、blanch less thanとかがちゃんと動いていないことを発見

lb辺りのバグを修正
mulを2クロックにする
asmを弄るときにsubをぶっ壊してしまい、動かなくなってしまった
asmを弄るときはmifファイルの差分に注意するべき！

9/20
mulのテストをした
ipが少しバグっていた（出力の制限をすると下ではなく上をとるみたい）
mulを直して動作を確認
...
mulhがバグっていたことが判明
修正
divのsoft float実装が動いた
マンデルブロを実装した
fmaddのアセンブリを書いて動作を検証した
reg writeを忘れていたことを修正
fnmsubの符号について勘違いしていたが、asmの実装を少し間違えていたので正常に動作した

todo
マンデルブロを実機で動かす  

9/23
fdivの実装をllvmのライブラリから取ってききた
マンデルブロを動かしてみた
マンデルブロがprintintとかを入れるとちゃんと動くが、そのままだと動かないという問題が発生した
アセンブリをいじりまくってエスパーした
floatの比較の結果をinteger registerのみならずfloatのレジスタにも書き込んでしまっていたのが原因と解明
修正して小さなマンデルブロが動いた
uartのバッファが足りないので、fifoがemptyがmemory readで判定できるようにして、ソフトウェア側で送信されるまで待つみたいな実装をしたい

9/27
SDRAMのデモンストレーションを動かしてみた
謎にコンパイルしたら動いたようである
IPの動きを確認したい
sdramはipではなく直接verilogコードを書いているように見える 
適切にコピペすることで動かすことが出来ないだろうか？

10/1
とりあえずパイプライン全体を止められるようにしてみる
uartをそれに対応させて動くか確認する
stateマシンを書き始めた
...
sendも動いた
でかいマンデルブロを描画してみる
メモリコントローラと繋げるためのmemoryの設計
メモリコントローラと繋げてみる

10/8
controlerのバグの修正
なんと括弧の対応が取れていないのにコンパイルエラーを吐かなかった
何がどうなっているのか
ちゃんとしたformatterがあった方が良かった…
256*256のマンデルブロ集合の出力に成功した

10/10
SDRAMのテストツールをコピペして最小限に動かすことに成功した。
テスト部分を少し書き換えて、cpuコアに入れ込む事を試みることにする
onchip memoryとは別のメモリ領域としてつかうみたいな感じにする
実行は出来ない（後々出来るようにしても良いかもしれないけど）

10/12
メモリのclkとその他のclkがある
異なるclk間はfifoで繋ぎたいのだが、少し問題があって今の状態だとaddressは間にfifoが挟まっていない
とりあえずbaseadressが16の状態で繋いで、datapathが正常に動いているかを確認するか？